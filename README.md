...
- The Application can still be broken in various ways and is yet to be fixed. e.g. Changing chat mid token streaming, Giving user input during token generation, ~~newer versions of streamlit produces ghost widgets (can be fixed with downgrading package but this also leads to other issues)~~ etc..
